...
* Eight distinct box styles: Modern, Academic, Minimal, Card, Ruled, Editorial, Summary, and Bulletin
...
* Fully configurable colour scheme with six colour pickers and eleven one-click colour presets
...
* Native Gutenberg block with inspector controls and per-block style override
* List styles (ul/ol) rendered correctly inside box content, matching frontend in the block editor
...
= 2.3.3 =
* Added: Box Width setting — Full width (default) or Compact (max-width 520 px, centred). Available under Appearance → Typography & Shape. Live preview updates immediately. Applied via abstract-box--compact modifier class.
* Fixed: bulletin style was missing from STYLE_CLASSES in admin-settings.js, causing it not to be cleared on style switch in the live preview.

= 2.3.2 =
* Fixed: Bulletin style had no padding-top or padding-left — title and content were flush against the border.
* Changed: All styles now use shared --ab-pad-v (20px) and --ab-pad-h (24px) CSS custom properties for consistent inner spacing. Card and Summary add a small increment for their respective visual characters.

= 2.3.1 =
* Added: Bulletin colour preset — flat white + red accent, sets Bulletin style.
* Added: Editorial colour preset — flat white + slate accent, sets Editorial style.
* Added: Summary colour preset — violet tint + purple accent, sets Summary style.
* Changed: Preset count increased from 8 to 11.

= 2.3.0 =
* Removed: Bulletin Stripe colour option — redundant with Accent Colour. Bulletin border and stripe always use the accent colour.
* Kept: Bullet Dots colour remains as an independent option.
